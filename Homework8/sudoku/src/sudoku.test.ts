...
describe('sudoku', () => {  
    // Initial configuration (game instance, necessary keys and addresses).
    ...
    it('accepts a correct solution', async () => {
        ...
        -) Deploy the zkApp;
        -) Checking if the sudoku is solved (expecting to be false);
        -) Calling the "solveSudoku" function (off-chain computation);
        -) Submitting the solution to the zkApp by building a Mina tx.
        -) Generating a proof for this transaction, signing it and sending to the Mina Blockchain.
        -) Checking if the sudoku is solved (expecting to be true);
        */
    ...
    it('rejects an incorrect solution', async () => {
        /*
        The flow is the same as above, but we pass the incorrect solution.
        */
        await deploy(zkApp, zkAppPrivateKey, sudoku, sender, senderKey);
